...
setwd(x) #指定工作目录
  ...
  for(i in 1:length(collapse_miRNA)){
    #3、批量处理-miR-|-mir-|-let-,提取出不含有关键词的全部行成为新的数据框
    notmml <-  subset(collapse_miRNA[[i]],!grepl("-miR-|-mir-|-let-",Annotation))  #取出不包含某些含-miR-或者-mir-或-let-特征关键词的行
    #---------------------4、批量处理-3p|-5p|miR，求和reads
    #提取出含有关键词的行
    mat <- "((-3p)$|(-5p)$|miR)"
    mi <- list() #存储变换后的y值
    #按;分隔字符串，stri_reverse是stringi包的函数
    y=lapply(stri_split_regex(stri_reverse(collapse_miRNA[[i]]$Annotation), pattern = '[;\\s]+', n = 150), stri_reverse)  # #按;分隔字符串，stri_reverse是stringi包的函数
    for(j in 1:length(y)){
    ...
    re=collapse_miRNA[[i]][,]  #取re文件内的行和列
    #filename含有列表命名
    write.table(re,filename2[i],sep = "\t",row.names = FALSE,quote = F)  #row.names = FALSE去掉序号一列
  }
}
